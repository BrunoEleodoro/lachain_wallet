...
class AppsPage extends StatefulWidget with GetItStatefulWidgetMixin {
  AppsPage({Key? key, required this.web3App}) : super(key: key);
      ...
      children: [
        ...
        ElevatedButton(
            onPressed: () async {
              ...
              print('PAIRING TOPIC: ${connectResponse.pairingTopic}');
              final Uri uriData = Uri.parse(connectResponse.uri!.toString());
              _web3Wallet.pair(uri: uriData).then((res) {
                print('PAIRING TOPIC 222: ${connectResponse.pairingTopic}');
                print(res);
                var pairing =
                    _web3Wallet.pairings.get(connectResponse.pairingTopic);
                print('PAIRING: $pairing');
              }).catchError((e) {
                print('ERROR: $e');
              });

              await connectResponse.session.future.then((session) {
                print(session);
                print('SADASD');
...
//                     children: [
//                       _buildActionButton(Icons.send, 'Enviar', () async {
//                         // _onScanQrCodeSend();
...
//                         print(connectResponse);
//                         print('PAIRING TOPIC: ${connectResponse.pairingTopic}');
//                         _web3Wallet.pair(uri: connectResponse.uri!);
